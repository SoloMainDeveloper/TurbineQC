...
void FileSystem::writeCompareFLR(QString fullFileName, QList<ResultCompare2Params*> result, double precision) {
	...
	out << QString("%1 %2 %3 %4 %5 %6 %7\n")
		...
		.arg(QString("-").repeated(10), -10)
		.arg(QString("-").repeated(10), -10)
		.arg(QString("-").repeated(10), -10)
		.arg(QString("-").repeated(20), -20);

	double maxDiffDiameter = 0.0;
	double maxErrorDiameter = 0.0;

	double maxDiffChord = 0.0;
	double maxErrorChord = 0.0;

	double maxDiffLEWidth = 0.0;
	double maxErrorLEWidth = 0.0;

	double maxDiffTEWidth = 0.0;
	double maxErrorTEWidth = 0.0;

	double maxDiffMinX = 0.0;
	double maxErrorMinX = 0.0;

	double maxDiffDev = 0.0;
	double maxErrorDev = 0.0;

	double paramsSumError = 0.0;
	double paramsCounter = 0.0;

	double devsSumError = 0.0;
	double devsCounter = 0.0;
	...
	for (auto i = 0; i < result.count(); i++) {
		out << QString("%1 %2 %3 %4 %5 %6 %7\n")
			...
			.arg(result[i]->ut(), -10)
			.arg(result[i]->lt(), -10)
			.arg(result[i]->error(), -10)
			.arg(result[i]->diff() > precision ? "FAIL" : "OK", -5);

		if (result[i]->name1().contains("Maximum")) {
			if (maxDiffDiameter < result[i]->diff()) {
				maxDiffDiameter = result[i]->diff();
			}

			if (maxErrorDiameter < result[i]->error()) {
				maxErrorDiameter = result[i]->error();

			}

			if (result[i]->error() > 0.0) {
				paramsSumError += result[i]->error();
				paramsCounter++;
			}
		}

		if (result[i]->name1().contains("Chord")) {
			if (maxDiffChord < result[i]->diff()) {
				maxDiffChord = result[i]->diff();
			}

			if (maxErrorChord < result[i]->error()) {
				maxErrorChord = result[i]->error();
			}
		...
		if (result[i]->name1().contains("LE_Thck")) {
			if (maxDiffLEWidth < result[i]->diff()) {
				maxDiffLEWidth = result[i]->diff();
			}

			if (maxErrorLEWidth < result[i]->error()) {
				maxErrorLEWidth = result[i]->error();
			}
		...
		if (result[i]->name1().contains("TE_Thck")) {
			if (maxDiffTEWidth < result[i]->diff() && result[i]->diff() < 1) {
				maxDiffTEWidth = result[i]->diff();
			}

			if (maxErrorTEWidth < result[i]->error() && result[i]->diff() < 1) {
				maxErrorTEWidth = result[i]->error();
			}

			if (result[i]->error() > 0.0 && result[i]->diff() < 1) {
				paramsSumError += result[i]->error();
				paramsCounter++;
			}
		}

		if (result[i]->name1().contains("MinX")) {
			if (maxDiffMinX < result[i]->diff()) {
				maxDiffMinX = result[i]->diff();
			}

			if (maxErrorMinX < result[i]->error()) {
				maxErrorMinX = result[i]->error();
			}
		...
		if (result[i]->name1().contains("GlobalC")) {
			if (maxDiffDev < result[i]->diff()) {
				maxDiffDev = result[i]->diff();
			}

			double error = result[i]->diff() / 0.2 * 100;

			if (maxErrorDev < error) {
				maxErrorDev = error;
			}

			if (result[i]->error() > 0.0) {
				devsSumError += result[i]->error();
				devsCounter++;
			}
		}
	}

	qDebug() << "maxDiffDiameter: " << maxDiffDiameter;
	qDebug() << "maxErrorDiameter: " << maxErrorDiameter;

	qDebug() << "maxDiffChord: " << maxDiffChord;
	qDebug() << "maxErrorChord: " << maxErrorChord;

	qDebug() << "maxDiffLEWidth: " << maxDiffLEWidth;
	qDebug() << "maxErrorLEWidth: " << maxErrorLEWidth;

	qDebug() << "maxDiffTEWidth: " << maxDiffTEWidth;
	qDebug() << "maxErrorTEWidth: " << maxErrorTEWidth;

	qDebug() << "maxDiffMinX: " << maxDiffMinX;
	qDebug() << "maxErrorMinX: " << maxErrorMinX;

	qDebug() << "maxDiffDev: " << maxDiffDev;
	qDebug() << "maxErrorDev: " << maxErrorDev;

	qDebug() << ((paramsSumError / paramsCounter) + (devsSumError / devsCounter));
