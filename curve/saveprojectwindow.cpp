...
SaveProjectWindow::SaveProjectWindow(Project *project, QWidget *parent) : QDialog(parent), _ui(new Ui::SaveProjectWindow()) {
    ...
    connect(_ui->openButton, &QPushButton::clicked, this, &SaveProjectWindow::openExplorer);
    connect(_ui->projectNameEdit, &QLineEdit::textChanged, this, &SaveProjectWindow::projectNameChanged);
}

void SaveProjectWindow::initialization() {
    exec();
}

void SaveProjectWindow::closeWindow() {
    close();
...
void SaveProjectWindow::closeEvent(QCloseEvent *event) {
    closeWindow();
}

void SaveProjectWindow::showBoxCRV() {
...
void SaveProjectWindow::saveProject() {
...
void SaveProjectWindow::openExplorer() {
...
void SaveProjectWindow::projectNameChanged() {
    auto name = _ui->projectNameEdit->text();
    if(name != "") {
