...
PrintPreviewDialog::PrintPreviewDialog() : _ui(new Ui::PrintPreviewDialog)
{
    ...
    this->setWindowTitle("Средство просмотра печати");
    _printer = &Printer::instance();

    _ui->pageList->setHeaderHidden(true);
    _printItem = new QAction(QIcon("icons/printer.ico"), "Печать страницы");
    _deleteItem = new QAction(QIcon("icons/delete.ico"), "Удалить");
...
void PrintPreviewDialog::printHandler()
{
    ...
    for(auto i = 0; i < _ui->pageList->topLevelItemCount(); i++) {
        ...
        if(item->checkState(0) == Qt::Checked) {
            auto index = textItem.split("Страница ")[1].toInt() - 1;
...
void PrintPreviewDialog::printItemTriggered()
{
    ...
    auto index = currentItem->text(0).split("Страница ")[1].toInt() - 1;
...
void PrintPreviewDialog::addPage(const QMap<QString, QString>& information)
{
    ...
    auto item = new QTreeWidgetItem(_ui->pageList, { QString("Страница %1").arg(pageNumber) });
    item->setFlags(item->flags() | Qt::ItemIsUserCheckable);
    item->setCheckState(0, Qt::Checked);

    auto nominalCurve = QString("Номинальный профиль: %1\n").arg(information["nominalCurve"]);
    auto measuredCurve = QString("Измеренный профиль: %1\n").arg(information["measuredCurve"]);
    auto creationTime = QString("Время создания: %1").arg(information["time"]);
...
void PrintPreviewDialog::updateReportPages(int updateIndex)
{
    for(auto i = updateIndex; i < _ui->pageList->topLevelItemCount(); i++) {
        ...
        currentItem->setText(0, QString("Страница %1").arg(i + 1));
    }
}
