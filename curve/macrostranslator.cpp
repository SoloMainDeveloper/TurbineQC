...
QList<std::shared_ptr<ICommand>>* MacrosTranslator::translateCRM(const QString& operationText)
{
    ...
    for(auto i = 0; i < operations.size(); i++) {
        ...
        auto command = parseCRMCommand(operationText[0].split(',').first(), operationText);
        if(command) {
...
std::shared_ptr<ICommand> MacrosTranslator::parseCRMCommand(QString operationTypeStr, QStringList operationText)
{
    auto operationTypeCRM = operationCRMFromString(operationTypeStr);
        ...
        case MacrosTranslator::ShowOptions: {
            ...
            if(secondLine[1] == "Visible") {
                ...
                return std::make_shared<ChangeCurveParametersCommand>(
                    figureName, isVisible, showPoints, connectPoints, showVectors,
                ...
                return std::make_shared<UnknownCommand>(operationTypeStr);
                // std::make_shared<>();
            }
            else {
                bool isVisible = secondLine[2] == "1";
                return std::make_shared<ChangeFigureVisibilityCommand>(figureName, isVisible);
            }
        }
