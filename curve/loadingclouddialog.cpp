...
#include "ui_loadingclouddialog.h"
#include "filesystem.h"
#include "project.h"

LoadingCloudDialog::LoadingCloudDialog() : _ui(new Ui::LoadingCloudDialog()) {
    _ui->setupUi(this);
    ...
    connect(_ui->fileBtn, &QRadioButton::clicked, this, &LoadingCloudDialog::selectFile);
    connect(_ui->clipboardBtn, &QRadioButton::clicked, this, &LoadingCloudDialog::selectClipboard);
    connect(_ui->findFileBtn, &QPushButton::clicked, this, &LoadingCloudDialog::findFile);
    connect(_ui->openFileBtn, &QPushButton::clicked, this, &LoadingCloudDialog::openFile);
    connect(_ui->loadCloudBtn, &QPushButton::clicked, this, &LoadingCloudDialog::loadCloud);
    connect(_ui->closeWindowBtn, &QPushButton::clicked, this, &LoadingCloudDialog::closeWindow);
    connect(_pasteDataBtn, &QPushButton::clicked, this, &LoadingCloudDialog::pasteData);
}

void LoadingCloudDialog::initialize() {
    exec();
}

void LoadingCloudDialog::selectFile() {
    _ui->groupOfFileWidgets->show();
    _groupOfBtnWidgets->hide();
}

void LoadingCloudDialog::selectClipboard() {
    _groupOfBtnWidgets->show();
    _ui->groupOfFileWidgets->hide();
}

void LoadingCloudDialog::findFile() {
...
bool LoadingCloudDialog::isCorrectFileExtension(const QString &filePath) {
    auto extension = QFileInfo(filePath).suffix().toLower();
    return extension == "txt" || extension == "nom";
}

void LoadingCloudDialog::openFile() {
...
void LoadingCloudDialog::pasteData() {
    QApplication::setOverrideCursor(Qt::WaitCursor);
...
void LoadingCloudDialog::loadCloud() {
    ...
    if(project->containsFigure(name)) {
    ...
    try {
        FileSystem::loadCloud(_ui->filePathLineEdit->text(), name, separator, skipStart,
...
void LoadingCloudDialog::closeEvent(QCloseEvent * event) {
    closeWindow();
}

void LoadingCloudDialog::closeWindow() {
    this->close();
...
LoadingCloudDialog::~LoadingCloudDialog() {
    delete _ui;
}
