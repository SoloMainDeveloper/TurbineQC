    ...
    : _ui(new Ui::MacrosDialog())
{
    ...
    _macrosReaders = new QMap<QString, IMacrosReader*>();
    _macrosReaders->insert("crm", new MacrosReaderCRM());
    _macrosReaders->insert("json", new MacrosReaderJSON());
    ...
    connect(_macrosManager, &MacrosManager::operationLogged, this, &MacrosDialog::onOperationLogged);
    connect(_macrosManager, &MacrosManager::recordingToggled, this, &MacrosDialog::onRecordingToggled);
...
void MacrosDialog::insert(int index, QTreeWidgetItem* itemToAdd)
{
    ...
    while(index != _operationList->topLevelItemCount()) {
        ...
        auto text = itemToTake->text(0);
        items.append(itemToTake);
    }
...
void MacrosDialog::load()
{
    ...
    if(!path.isEmpty()) {
        ...
        QList<int> unknownCommands;
        if(_macrosReaders->contains(extension)) {
            QList<std::shared_ptr<ICommand>>* commands = _macrosReaders->value(extension)->read(path);
            int commandsCount = commands->length();

            auto isRecording = _macrosManager->isRecording();
            _macrosManager->setRecording(true);
            for(auto i = 0; i < commandsCount; i++) {
                auto command = commands->at(i);
                _macrosManager->log(command);

                if(command.get()->getType() == CommandType::Unknown) {
                    unknownCommands.append(i + 1);
                }
            }
            _macrosManager->setRecording(isRecording);

            setWindowTitle(tr("Macros: ") + fileInfo.baseName());

            int successfullCommandsCount = commandsCount - unknownCommands.length();
            QString commandsInfo = QString::number(successfullCommandsCount) + " / " + QString::number(commandsCount);
            QString resultInfo = tr("Successfully interpreted commands: ") + commandsInfo + "\n";

            for(auto unknownCommandIndex : unknownCommands) {
                QString unkownCommandLine = tr("Error while interpretation: %1\n").arg(unknownCommandIndex);
                resultInfo.append(unkownCommandLine);
            }
            resultInfo.removeLast();

            DialogService::instance().showInfo(tr("Macros was loaded"), resultInfo);
        }
        else {
            DialogService::instance().showWarning(tr("Error"), tr("Cant translate extension: ") + extension);
        }
    }
}
