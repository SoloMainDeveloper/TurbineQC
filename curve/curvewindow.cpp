    ...
    : QMainWindow(parent), _ui(new Ui::CurveWindow)
{
    ...
    connect(_ui->actionLoadFile, &QAction::triggered, this, [this]() {
        _dialogService->show(DialogType::LoadCloud);
    });
    connect(_ui->actionAssignTolerancesToNominal, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::AirfoilTolerances);
    });
    connect(_ui->actionCalculateTurbineParameters, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::Turbine);
    });
    connect(_ui->actionCalculateEdgeWidth, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::WidthEdge);
    });
    connect(_ui->actionMergeClouds, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::MergePointCloud);
    });
    connect(_ui->actionRadiusCorrection, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::RadiusCorrection);
    });
    connect(_ui->actionExportToFLR, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::ExportToFLR);
    });
    connect(_ui->actionShowPrintViewer, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::PrintPreview);
    });
    connect(_ui->actionBestFit, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::BestFit);
    });
    connect(_ui->actionCalculateDeviations, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::CalculateDeviations);
    });
    connect(_ui->actionShowMacrosWindow, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::Macros);
    });
    connect(_ui->actionShift, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::Shift);
    });
    connect(_ui->actionRotate, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::Rotate);
    });
    connect(_ui->actionAlignment, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::Alignment);
    });
    connect(_ui->actionCurve_2D_calculation, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::CalculateCurve);
    });
    connect(_ui->actionEdit_Part_Data, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::PartData);
    });
    connect(_ui->actionText, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::InsertText);
    });
    connect(_ui->actionCompareFLR, &QAction::triggered, [this]() {
        _dialogService->show(DialogType::CompareFLR);
    });
    ...
    connect(_ui->actionLoadProject, &QAction::triggered, this, [&]() {
        auto projectPath = QFileDialog::getOpenFileName(nullptr, tr("Open project"), "", "(*.txt *.crv)");
        try {
    ...
    _magnificationLabel = new QLabel(tr("Scale: 1"));
    ...
    connect(_project, &Project::scaleChanged, [&](double scale) {
        _magnificationLabel->setText(QString(tr("Scale: %1")).arg(scale, 0, 'f', _project->precision()));
    });
