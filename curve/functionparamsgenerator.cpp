...
Function6Params FunctionParamsGenerator::params6(std::shared_ptr<ReportSettings> reportSettings, bool isClosed, bool isGlobal)
{
    auto bestFitType = reportSettings->bestFitType();
    auto params = reportSettings->turbineParameters();

    auto xShift = params.value(TurbineParameter::Type::ShiftX, QList<TurbineParameter*>());
    auto yShift = params.value(TurbineParameter::Type::ShiftY, QList<TurbineParameter*>());
    auto turn = params.value(TurbineParameter::Type::Turn, QList<TurbineParameter*>());
    ...
    if(bestFitType == ReportSettings::BestFitType::OnlyTranslation) {
        ...
        if(isGlobal && !xShift.isEmpty() && !yShift.isEmpty() && !turn.isEmpty()) {
            double xShiftLT = xShift.first()->LT;
            double xShiftUT = xShift.first()->UT;
            bool needXShift = xShift.first()->extraParam2.toInt() == 1 ? true : false;

            double yShiftLT = yShift.first()->LT;
            double yShiftUT = yShift.first()->UT;
            bool needYShift = yShift.first()->extraParam2.toInt() == 1 ? true : false;

            double turnLT = turn.first()->LT;
            double turnUT = turn.first()->UT;
            bool needTurn = turn.first()->extraParam2.toInt() == 1 ? true : false;

            return Function6Params(true, Function6Params::Algorithm::Curve, isClosed, true, true, true,
                needXShift, xShiftLT, xShiftUT,
                needYShift, yShiftLT, yShiftUT,
                needTurn, turnLT, turnUT);
        }
        return Function6Params(true, Function6Params::Algorithm::Curve, isClosed, true, true, true);
    }
    else {
