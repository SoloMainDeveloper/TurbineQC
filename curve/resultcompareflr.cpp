	...
	double upperTolerance, double lowerTolerance)
{
	...
	_error = _diff / (upperTolerance - lowerTolerance) * 100;
...
bool ResultCompare2Params::isDiffBiggerThanTolerance(double tolerancePercent)
{
	auto toleranceSum = abs(_upperTolerance - _lowerTolerance);

	if (toleranceSum == 0) {
		toleranceSum = 0.5;
	}

	auto precision = toleranceSum * tolerancePercent / 100;

	return isDiffBiggerThanPrecision(precision);
}
...
double ResultCompare2Params::ut()
{
	return _upperTolerance;
}

double ResultCompare2Params::lt()
{
	return _lowerTolerance;
}

double ResultCompare2Params::error()
{
	return _error;
}
