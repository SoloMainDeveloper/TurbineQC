...
AirfoilTolerancesDialog::AirfoilTolerancesDialog() : _ui(new Ui::AirfoilTolerancesDialog())
{
    ...
    addToleranceBlock(_constantTolerancesLayout, "Допуски (Верхний / Нижний)", _toleranceLabel, _upperToleranceLineEdit, _lowerToleranceLineEdit);
    _constantTolerancesLayout->addStretch();

    QPushButton* okButton = new QPushButton("ОК");
    QPushButton* cancelButton = new QPushButton("Отменить");
    _constantToleranceButtonBox->addButton(okButton, QDialogButtonBox::AcceptRole);
    _constantToleranceButtonBox->addButton(cancelButton, QDialogButtonBox::RejectRole);
    _constantToleranceButtonBox->setCenterButtons(true);
    //_constantToleranceButtonBox->addButton(QDialogButtonBox::Ok);
    //_constantToleranceButtonBox->button(QDialogButtonBox::Ok)->setEnabled(false);
    //_constantToleranceButtonBox->addButton(QDialogButtonBox::Cancel);
    connect(_constantToleranceButtonBox, &QDialogButtonBox::accepted, this, &AirfoilTolerancesDialog::assignTolerances);
    connect(_constantToleranceButtonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
    _constantTolerancesLayout->addWidget(_constantToleranceButtonBox);
...
void AirfoilTolerancesDialog::changeCurrentMode()
{
    ...
    if(currentMode == "Константный допуск") {
...
void AirfoilTolerancesDialog::assignTolerances()
{
    ...
    if(currentMode == "Константный допуск") {
        ...
        BladeGeometryService::calculateConstantTolerances(_currentCurveName, upperTolerance, lowerTolerance);
    }
    else if(currentMode == "Edges") {
        auto lEDir = 0;
        ...
        BladeGeometryService::calculateEdgesTolerance(_currentCurveName, lEDir, lEPercent, tEPercent, lEUpper, lELower, tEUpper, tELower, highEUpper, highELower,
