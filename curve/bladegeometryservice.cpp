...
void BladeGeometryService::enlargeCurveWithIntermediatePoints(QString figureName, const Function1Params* params, Project* project)
{
...
void BladeGeometryService::calculateCurve(QString figureName, QString newFigureName, const Function1Params* params, Project* project)
{
	auto figure = project->findFigure(figureName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
void BladeGeometryService::regenerateCurve(QString figureName, QString newFigureName, const Function19Params* params)
{
...
void BladeGeometryService::createMiddleCurve(QString parentName, QString figureName, const Function18Params* params, Project* project, QColor color)
{
...
CurveFigure BladeGeometryService::getMiddleCurve(QString curveName, const Function18Params* params, Project* project)
{
...
double BladeGeometryService::getMiddleCurveLength(QString curveName, const Function18Params& params)
{
...
void BladeGeometryService::createContactLine(QString parentName, QString figureName, const Function18Params* params, Project* project, QColor color)
{
...
LineFigure BladeGeometryService::getContactLine(QString curveName, const Function18Params* params, Project* project)
{
	auto figure = project->findFigure(curveName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
void BladeGeometryService::createMaxCircle(const QString& parentName, const Function18Params& params, const QString& figureName, QColor color)
{
...
CircleFigure BladeGeometryService::getMaxCircle(const QString& curveName, const Function18Params& params)
{
	auto figure = Project::instance().findFigure(curveName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
QPair<PointFigure, PointFigure> BladeGeometryService::getChord(const QString& curveName, const Function18Params& params)
{
	auto figure = Project::instance().findFigure(curveName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
double BladeGeometryService::getChordLength(QString curveName, const Function18Params* params, Project* project)
{
	auto figure = project->findFigure(curveName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
QPair<CurvePoint, CurvePoint> BladeGeometryService::getWidthOfLeadingEdge(const QString& curveName,
...
QPair<CurvePoint, CurvePoint> BladeGeometryService::getWidthOfTrailingEdge(const QString& figureName,
	const Function18Params& params, double distanceFromEdge)
{
	auto figure = Project::instance().findFigure(figureName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
CircleFigure BladeGeometryService::getRadiusOfLeadingEdge(const QString& curveName, const Function18Params& params, double angleInDegrees)
{
	auto figure = Project::instance().findFigure(curveName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
CircleFigure BladeGeometryService::getRadiusOfTrailingEdge(const QString& figureName, const Function18Params& params, double angleInDegrees)
{
	auto figure = Project::instance().findFigure(figureName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
double BladeGeometryService::getMinX(const QString& figureName, const Function18Params& params)
{
	auto figure = Project::instance().findFigure(figureName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
void BladeGeometryService::makeRadiusCorrection(QString figureName, QString figureNewName, const Function3Params* params)
{
...
bool BladeGeometryService::tryMergePointClouds(QString firstCurveName, QString secondCurveName, QString resultName, double threshold, bool needSorted)
{
	auto& project = Project::instance();
...
void BladeGeometryService::calculateDeviations(QString nomCurveName, QString measCurveName, QString resultCurveName, const Function4Params* params)
{
	auto& project = Project::instance();
...
void BladeGeometryService::calculateBestFit(QString nomCurveName, QString measCurveName, QString resultCurveName, QString bestFitLineName, const Function6Params* params)
{
	auto& project = Project::instance();
...
void BladeGeometryService::calculateBestFit(QString nomCurveName, QString measCurveName, QString resultCurveName, QString bestFitLineName, const Function21Params* params)
{
	auto& project = Project::instance();
	auto nominalFigure = project.findFigure(nomCurveName);
	auto measuredFigure = project.findFigure(measCurveName);
...
void BladeGeometryService::calculateConstantTolerances(QString figureName, double upperTolerance, double lowerTolerance)
{
...
void BladeGeometryService::calculateEdgesTolerance(QString figureName, int leadingEdgeDirection, double lEPercent, double tEPercent, double lEUpper, double lELower,
	double tEUpper, double tELower, double highEUpper, double highELower, double lowEUpper, double lowELower, Project* project)
{
	...
	if (f18result.curveLE.points().length() + f18result.curveHigh.points().length() + f18result.curveTE.points().length() + f18result.curveLow.points().length() == curve->points().length() + 8) {
		...
		pointsWithTolerances = f18result.curveLE.points() + f18result.curveLow.points() + f18result.curveTE.points() + f18result.curveHigh.points();
	}
...
void BladeGeometryService::insertBestFitDimension(const QString& figureName, const QString& parentName, double x, double y, double z, bool isShowX, bool isShowY, bool isShowR)
{
...
CurveParts BladeGeometryService::divideCurveIntoParts(const QString& figureName, const Function18Params* params, Project* project)
{
	auto figure = project->findFigure(figureName);
	auto curve = dynamic_cast<const CurveFigure*>(figure);
...
void BladeGeometryService::calculateStretch(const QString& nomCurveName, const QString& measCurveName, const QString& resultCurveName, const Function31Params* params31, const Function6Params* params6, Project* project)
{
	auto nominalFigure = project->findFigure(nomCurveName);
	auto nominalCurve = dynamic_cast<const CurveFigure*>(nominalFigure);
	auto measuredFigure = project->findFigure(measCurveName);
...
void BladeGeometryService::calculateCurveUsing3DVectors(const QString& nomCurveName, const QString& measCurveName, const QString& resultCurveName, const Function42Params* params, double radiusCorrection, Project* project)
{
	auto nominalFigure = project->findFigure(nomCurveName);
	auto nominalCurve = dynamic_cast<const CurveFigure*>(nominalFigure);
	auto measuredFigure = project->findFigure(measCurveName);
...
void BladeGeometryService::calculateCurveUsing3DVectorsTest(const QString& nomCurveName, const QString& measCurveName, const QString& resultCurveName,
	const Function42Params* params, double radiusCorrection)
{
	...
	auto nominalPoints = nominalCurve->points();
	auto measuredPoints = measuredCurve->points();
	auto resultPoints = CurveMachine::calculateCurveUsing3DVectorsTest(nominalPoints, measuredPoints, *params, radiusCorrection);

	auto resultCurve = new CurveFigure(resultCurveName, resultPoints);
	project.safeInsert(resultCurveName, resultCurve);

	MacrosManager::instance().log(std::make_shared<RadiusCorrection3DCommand>(
		nomCurveName, measCurveName, resultCurveName, params, radiusCorrection));
}

void BladeGeometryService::calculateMeasuredParams(std::shared_ptr<ReportSettings> reportSettings, const QString& measuredProfileName)
{
...
void BladeGeometryService::calculateMeasuredTransformParams(std::shared_ptr<ReportSettings> reportSettings, const QString& parentLineName)
{
...
double BladeGeometryService::getDistanceBetweenPoints(Point firstPoint, Point secondPoint)
{
	return CurveMachine::getDistanceBetweenPoints(firstPoint, secondPoint);
}

Point BladeGeometryService::getMiddlePoint(Point firstPoint, Point secondPoint)
{
	return CurveMachine::getMiddlePoint(firstPoint, secondPoint);
}

void BladeGeometryService::reassembleWholeCurve(const QString& resultCurveName, const CurveParts& curveParts, LEDirection direction, Project* project)
{
...
void BladeGeometryService::reassembleCurveWithoutTE(const QString& resultCurveName, const CurveParts& curveParts, LEDirection direction, Project* project)
{
...
void BladeGeometryService::reassembleCurveWithoutEdges(const QString& resultCurveName, const CurveParts& curveParts, LEDirection direction, Project* project)
{
...
ResultCompareFLR* BladeGeometryService::compareFLR(QString filepathFLR1, QString filepathFLR2, QString resultPath, int pointsStartIndex, double precision)
{
...
QList<ResultCompare2Params*> BladeGeometryService::compareOnlyDimensionsFLR(QString filepathFLR1, QString filepathFLR2, int pointsStartIndex)
{
	ARGUMENT_ASSERT(QFile::exists(filepathFLR1), "Compare FLR: FLR 1 not found");
	ARGUMENT_ASSERT(QFile::exists(filepathFLR2), "Compare FLR: FLR 2 not found");
	auto flr1 = FileSystem::readFile(filepathFLR1).split("$END$")[1].trimmed().split("\n");
...
ResultCompare2Params* BladeGeometryService::compareTwoParamsFLR(QString first, QString second)
{
