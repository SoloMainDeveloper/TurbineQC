...
Plot::Plot(QWidget* parent) : QCustomPlot(parent), _project(nullptr)
{
    ...
    xAxis->setRange(0, _defaultXRangeLength);
...
void Plot::setProject(Project* project)
{
    ...
    connect(project, &Project::zoomToPointRequested, this, &Plot::zoomToPoint);
...
{
    auto xShift = xAxis->range().size() / 2;
    auto yShift = yAxis->range().size() / 2;

    rescaleAxes(true);
    auto leftBound = xAxis->range().lower;
    auto rightBound = xAxis->range().upper;
    auto bottomBound = yAxis->range().lower;
    auto topBound = yAxis->range().upper;

    auto centerX = (leftBound + rightBound) / 2;
    auto centerY = (bottomBound + topBound) / 2;
    auto center = Point(centerX, centerY);

    xAxis->setRange(centerX - xShift, centerX + xShift);
    yAxis->setRange(centerY - yShift, centerY + yShift);

    auto xRangeMin = xAxis->range().lower;
    auto xRangeMax = xAxis->range().upper;
    auto yRangeMin = yAxis->range().lower;
    auto yRangeMax = yAxis->range().upper;

    auto xWidth = abs(rightBound - leftBound);
    auto yWidth = abs(topBound - bottomBound);
    if(yWidth / xWidth > _defaultYRangeLength / _defaultXRangeLength) {
        while(topBound < yRangeMax || bottomBound > yRangeMin) {
            yRangeMin = yAxis->range().lower;
            yRangeMax = yAxis->range().upper;
            zoomPlusToPoint(center, false);
        }
    }
    else {
        while(rightBound < xRangeMax || leftBound > xRangeMin) {
            xRangeMin = xAxis->range().lower;
            xRangeMax = xAxis->range().upper;
            zoomPlusToPoint(center, false);
        }
    }

    while(leftBound < xRangeMin || rightBound > xRangeMax || topBound > yRangeMax || bottomBound < yRangeMin) {
        xRangeMin = xAxis->range().lower;
        xRangeMax = xAxis->range().upper;
        yRangeMin = yAxis->range().lower;
        yRangeMax = yAxis->range().upper;
        zoomMinusToPoint(center, false);
    }

    layer("axes")->replot();
}

void Plot::zoomToPoint(double scaleFactor, const Point center)
{
    auto xShift = xAxis->range().size() / 2;
    auto yShift = yAxis->range().size() / 2;

    xAxis->setRange(center.x - xShift, center.x + xShift);
    yAxis->setRange(center.y - yShift, center.y + yShift);

    while(int(scaleFactor * 1000) > int(_magnitude * 1000)) {
        zoomPlusToPoint(center, false);
    }
    while(int(scaleFactor * 1000) < int(_magnitude * 1000)) {
        zoomMinusToPoint(center, false);
...
void Plot::zoomPlus()
{
    zoomPlusToPoint(centerPoint(), true);
}

void Plot::zoomPlusToPoint(const Point& point, bool needToReplot)
{
    auto magnitude = _magnitude * pow(_reductionFactor, -1);
    emit rescaled(magnitude, point, needToReplot);
}

void Plot::zoomMinus()
{
    zoomMinusToPoint(centerPoint(), true);
}

void Plot::zoomMinusToPoint(const Point& point, bool needToReplot)
{
...
void Plot::changeScale(double magnitude, const Point& point, bool needToReplot)
{
...
void Plot::drawCallout(const DimFigure* dimFigure)
{
    ...
    if(dimFigure->isShowTols()) {
        text += QString("*\n (%1 %2 %3)").arg(QString::number(value.nominal, 'f', _project->precision())).arg(QString::number(value.upperTolerance, 'f', _project->precision())).arg(QString::number(value.lowerTolerance, 'f', _project->precision()));
...
void Plot::onItemClicked(QCPAbstractItem* item, QMouseEvent* event)
{
    ...
    if(_project->findFigure(figureName) == nullptr)
        return;
...
QString Plot::getScreenshotInBase64(int width, int height, ReportSettings::Axis axisType)
{
    ...
    QString convertedImage = toBase64(image);

    return convertedImage;
}

QString Plot::toBase64(const QImage& image)
{
    ...
    auto convertedImage = QString::fromUtf8(byteArray.toBase64());
...
void Plot::onMouseWheel(QWheelEvent* event)
{
    if(event->angleDelta().y() > 0) {
        ...
        zoomPlusToPoint(mousePoint, true);
    }
    else if(event->angleDelta().y() < 0) {
        Point mousePoint = pixelToCoord(Point(event->position().x(), event->position().y()));
        zoomMinusToPoint(mousePoint, true);
    }
    _replotTimer.start();
}
...
void Plot::drawTable(const DimFigure* dimFigure)
{
    ...
    for(auto i = 0; i < visibleValues.length(); i++) {
        ...
        if(dimFigure->isShowTols()) {
            ...
            text = QString("(%1 %2 %3)").arg(QString::number(value.nominal, 'f', _project->precision())).arg(QString::number(value.upperTolerance, 'f', _project->precision())).arg(QString::number(value.lowerTolerance, 'f', _project->precision()));
            widthPx = fontMetrics.horizontalAdvance(text);
...
void Plot::drawDistanceBetweenCurvePoints(const DimFigure* dimFigure)
{
    ...
    if(dimFigure->isShowTols()) {
        labelText += QString("*\n (%1 %2 %3)").arg(QString::number(dimFigure->values()[0].nominal, 'f', _project->precision())).arg(QString::number(dimFigure->values()[0].upperTolerance, 'f', _project->precision())).arg(QString::number(dimFigure->values()[0].lowerTolerance, 'f', _project->precision()));
    ...
    double k;
    if((firstDimPoint.x - secondDimPoint.x) == 0)
        k = 0;
    else
        k = (firstDimPoint.y - secondDimPoint.y) / (firstDimPoint.x - secondDimPoint.x);
...
void Plot::drawCurveNumbering(const CurveFigure* curveFigure)
{
    if(curveFigure->isShowNumbering()) {
        ...
        if(numberingInterval > points.size())
            return;
